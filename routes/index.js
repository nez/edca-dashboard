    ...
    switch ( req.params.stage ){
        ...
        case 'tender':
            ...
            edca_db.tx(function (t) {
                ...
                //hitos
                var milestones = this.manyOrNone('select * from tendermilestones where contractingprcess_id = $1', [ req.params.cpid ]);
                //cambios
                var changes = this.manyOrNone('select * from tenderamendmentchanges where contractingprocess_id = $1' ,[ req.params.cpid ]);

                return this.batch( [ tender, documents, milestones, changes ] );
            }).then(function (data) {
                res.json({
                    "name": "Licitación",
                    "text":"",
        ...
        case 'award':
            ...
            edca_db.tx(function (t) {
                ...
                var suppliers = this.manyOrNone('select * from suppliers where contractingprocess_id = $1',[ req.params.cpid ]);
                //documentos
                var documents = this.manyOrNone('select * from awarddocuments where contractingprocess_id = $1', [ req.params.cpid ]);
                //items
                var items = this.manyOrNone('select * from awarditems where contractingprocess_id = $1', [ req.params.cpid ]);
                //cambios
                var changes = this.manyOrNone(' select * from awardamendmentchannges wheree contractingprocess_id = $1 ', [ req.params.cpid ]);
        ...
        case 'contract':
            ...
            edca_db.tx(function (t) {
                ...
                var items = this.manyOrNone('select * from contractitems where contractingprocess_id  =$1',[ req.params.cpid ]);
                ...
                res.json({
                    ...
                    "children": [
                        {
                            "name": "ID adjudicación","text": data[0].awardid //"LO-009KDH999-N50-2015"
                        },
                        {
                            "name": "ID contrato","text": data[0].contractid //"LPN-OP-DCAGI-SC-076/15"
                        },
                        {
                            "name": "Título","text": data[0].title //"Para los trabajos relativos a la construcción del drenaje pluvial temporal"
                        },
                        {
                            "name": "Descripción","text": data[0].description //"Construcción de tres drenes principales, tres canales existentes (parcialmente), tres plantas de bombeo, dos lagunas de regulación, veintidós alcantarilla y dos salidas al Dren General del Valle, al poniente del predio. Rehabilitación de caminos de acceso y puentes alcantarilla para permitir el paso de vehículos de una margen del dren a la margen opuesta."
                        },
                        {
                            "name": "Estatus","text": data[0].status//"Activo"
                        },
                        {
        ...
        case 'implementation':
            ...
            edca_db.tx(function (t) {
                ...
                var transactions = this.manyOrNone('', [ req.params.cpid ]);
                var milestones = this.manyOrNone('', [ req.params.cpid ]);
                var documents = this.manyOrNone('', [ req.params.cpid ]);
