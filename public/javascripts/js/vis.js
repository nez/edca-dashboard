...
BubbleChart = (function() {
    ...
    function BubbleChart(data) {
        ...
        // TAMAÑO DE LOS NODOS
        this.radius_scale = d3.scale.pow().exponent(0.4).domain([1, num_max_indicadores]).range([1, 150]);//.domain([1, 4.5]).range([10, 30]);

        this.create_nodes();
        this.create_vis();

        this.circles.style("fill",'#50852C');
    }
    ...
    BubbleChart.prototype.remove_colors = function() {
        this.circles.transition().duration(600).style("fill", "#50852C");
        hide_color_chart();
    };
...
$(function() {
    ...
    d3.csv("/data_bub.csv", render_vis);
